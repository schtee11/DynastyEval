...
const getHeroStats = (player, allPlayers) => {
  ...
  return [
    ...
    stats?.targets ? { label: 'Targets', value: stats.targets, tier: tier(stats.targets, peers.map(p => p.stats?.targets).filter(Boolean)) } : { label: 'Yds/Rec', value: stats?.receptions > 0 ? (stats.receivingYards / stats.receptions).toFixed(1) : null, tier: tier(stats?.receptions > 0 ? stats.receivingYards / stats.receptions : null, peers.map(p => p.stats?.receptions > 0 ? p.stats.receivingYards / p.stats.receptions : 0)) },
  ];
};
