...
const PlayerProfile = ({ player, allPlayers, studiedPlayers, toggleStudied, onBack, onSelectPlayer }) => {
  ...
  const getRadarData = useCallback(() => {
    ...
    if (pos === 'WR') {
      ...
      if (pData) {
        return [
          { stat: 'YPRR', value: computePercentile(pData.yprr, peers.map(p => p.receivingByPerspective?.[perspective]?.yprr)), fullMark: 100 },
          { stat: 'Tgt/RR', value: computePercentile(pData.tgtPerRR, peers.map(p => p.receivingByPerspective?.[perspective]?.tgtPerRR)), fullMark: 100 },
          { stat: '1D+TD/RR', value: computePercentile(pData.firstDownTDPerRR, peers.map(p => p.receivingByPerspective?.[perspective]?.firstDownTDPerRR)), fullMark: 100 },
          ...
          { stat: 'Rec YDs', value: computePercentile(pData.recYds || player.stats?.receivingYards, peers.map(p => p.receivingByPerspective?.[perspective]?.recYds || p.stats?.receivingYards)), fullMark: 100 },
        ];
      }
      return [
        ...
        { stat: 'Rec YDs', value: computePercentile(player.stats?.receivingYards, peers.map(p => p.stats?.receivingYards)), fullMark: 100 },
      ];
    }
    // TE
    return [
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
            <div>
              ...
              <h1 style={{
                ...
                fontSize: 36, color: 'var(--text-primary)', margin: '0 0 8px',
                lineHeight: 1.1,
              }}>
                {player.name}
              </h1>
      ...
      <div style={{
        ...
        <div className="stats-columns" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
          <div>
            {player.position === 'QB' && (
              <>
                <SectionLabel>Accuracy & Efficiency</SectionLabel>
                ...
                <StatRow label="INTs" value={player.stats?.INT} />
              </>
            )}
                    ...
                    <div style={{ display: 'flex', gap: 4, marginBottom: 12, flexWrap: 'wrap' }}>
                      {availablePerspectives.map(key => (
                        <button key={key} onClick={() => setPerspective(key)} style={{
                          ...
                          fontWeight: perspective === key ? 600 : 400,
                          padding: '4px 10px', border: '1px solid',
                          borderColor: perspective === key ? 'var(--accent)' : 'var(--border-primary)',
                          borderRadius: 'var(--radius-sm)', cursor: 'pointer',
                          background: perspective === key ? 'var(--accent-light)' : 'transparent',
                          color: perspective === key ? 'var(--accent-text)' : 'var(--text-secondary)',
          ...
          <div>
            {player.position === 'QB' && (
              <>
                ...
                <StatRow label="Scrambles" value={player.stats?.scrambles} allValues={peerVals(p => p.stats?.scrambles)} />
                <SectionLabel>Advanced</SectionLabel>
                <StatRow label="BTT Rate" value={player.stats?.bttRate} benchmark={4.0} unit="%" allValues={peerVals(p => p.stats?.bttRate)} />
                <StatRow label="TWP Rate" value={player.stats?.twpRate} unit="%" />
                <StatRow label="ADOT" value={player.stats?.adot} allValues={peerVals(p => p.stats?.adot)} />
                <StatRow label="Sacks" value={player.stats?.sacks} />
              </>
            )}
            ...
            {player.position === 'WR' && (
              <>
                <SectionLabel>Receiving Profile</SectionLabel>
